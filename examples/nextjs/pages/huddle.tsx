import { PinState, TrackReference, TrackReferenceOrPlaceholder } from '@livekit/components-core';
import {
  DisconnectButton,
  LiveKitRoom,
  LocalUserChoices,
...
const CustomGridLayout = () => {
  ...
  return (
    <div ref={gridContainerRef} className={styles.gridLayout}>
      <GridLayout tracks={tracks} style={{ width: 'auto', height: 'auto' }}>
        <TrackContext.Consumer>
          {(track) => track && <CustomParticipantTile trackRef={track} />}
        </TrackContext.Consumer>
      </GridLayout>
    </div>
  );
...
const CustomFocusLayout = ({ track }: { track: TrackReferenceOrPlaceholder }) => {
  return (
...
const CustomParticipantTile = ({ trackRef }: CustomParticipantTileProps) => {
  return (
    <ParticipantTile {...trackRef} className={styles.participantView}>
      <VideoTrack source={trackRef.source} className={styles.video}></VideoTrack>
            ...
            <div className={styles.roomLayout}>
              ...
              {layout === 'grid' ? (
                ...
                <LayoutContext.Consumer>
                  {(context) =>
                    context?.pin.state?.length && <CustomFocusLayout track={context.pin.state[0]} />
                  }
                </LayoutContext.Consumer>
              )}
