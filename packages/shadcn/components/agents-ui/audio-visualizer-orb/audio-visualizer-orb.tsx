...
import { OrbShader, type OrbShaderProps } from './shader';
...
export const AudioVisualizerOrbVariants = cva(['aspect-square'], {
  variants: {
    size: {
      icon: 'h-[24px] gap-[2px]',
      sm: 'h-[56px] gap-[4px]',
...
export interface AudioVisualizerOrbProps {
  state?: AgentState;
  audioTrack: TrackReferenceOrPlaceholder;
}

export function AudioVisualizerOrb({
  size = 'md',
  state = 'speaking',
  themeMode,
  rgbColor,
  ...
  audioTrack,
  className,
}: AudioVisualizerOrbProps & OrbShaderProps & VariantProps<typeof AudioVisualizerOrbVariants>) {
  ...
  return (
    <OrbShader
      ...
      brightness={1.0}
      className={cn(
        AudioVisualizerOrbVariants({ size }),
