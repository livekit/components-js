...
import { useRadialAnimator } from './hooks/useRadialAnimator';
...
export function AudioVisualizerRadial({
  ...
  className,
  children,
}: AudioVisualizerRadialProps & VariantProps<typeof AudioVisualizerRadialVariants>) {
  ...
  if (_barCount % 4 !== 0) {
    console.warn('barCount must be divisible by 4');
  }
  ...
  return (
    <div className={cn(AudioVisualizerRadialVariants({ size }), 'relative', className)}>
      {bands.map((band, idx) => {
